...
-- Drop existing helper functions to recreate them
...
-- This works with both Supabase's default auth and Clerk JWTs
CREATE OR REPLACE FUNCTION public.get_clerk_user_id() 
RETURNS TEXT AS $$
BEGIN
  -- Try to get from Supabase auth first (if using Supabase auth)
  IF auth.uid() IS NOT NULL THEN
    RETURN auth.uid()::text;
  END IF;
  
  -- Otherwise try to get from JWT claims (for Clerk integration)
...
-- Update the get_clerk_user_id function to check session variable first
CREATE OR REPLACE FUNCTION public.get_clerk_user_id() 
RETURNS TEXT AS $$
BEGIN
...
-- Drop all existing policies to recreate them with the new functions
-- Companies policies
...
COMMENT ON FUNCTION public.get_clerk_user_id() IS 'Extracts the Clerk user ID from the JWT token in the current request';
...
BEGIN
  ...
  RAISE NOTICE 'Make sure to configure the Clerk JWT template to include the user ID in the sub claim';
END $$;